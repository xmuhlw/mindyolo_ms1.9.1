# 可以对json中file_name,id,image_id以及对应的图片名称统一做修改
# 避免名称带字符串
# 0.从字典里面提取
# 一.json文件的操作
# 1.open json文件
...
def fix_coco_anno_json_imgid(input_path, start_id=0):
    ...
    convert_table, oldName_newId_dict = generate_and_repair_image_id(
        js, img_list, start_id)
...
def generate_and_repair_image_id(js, old_id_list, start_id=0):
    ...
    for i, img_dict in enumerate(old_id_list):
        ...
        convert_table.append(
            zip(new_name, new_id, img_dict['file_name'], img_dict['file_name'].split('.')[0]))
...
def fix_image_id_in_json_annotations(js, oldName_newId_dict):
    ...
    for i, ann_dict in enumerate(annotations):
        try:
            ...
            print(e)
            print("In oldName_newId_dict, we can't found key:",
                  ann_dict["image_id"])
...
def fix_image_jpg_xml_name_by_id_dict(xml_path=None, jpg_path=None, image_id_dict=None):
    ...
    if jpg_path is not None:
        ...
        #     else:
        #         print(f"Skipping as it's a directory.")
        # 将新文件夹里面的所有jpg文件更名
        ...
        for jpg_name in jpg_path_new_list:
            ...
            new_path_name = os.path.join(
                jpg_path_new, image_id_dict[jpg_name.split('.')[0]] + '.jpg')
...
if __name__ == '__main__':
    ...
    oldName_newId_dict = fix_coco_anno_json_imgid('instances_val2017.json')
    ...
    fix_image_jpg_xml_name_by_id_dict(
        None, '/home/ma-user/work/data9705-2/val/val2017', oldName_newId_dict)
